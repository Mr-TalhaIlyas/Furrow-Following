...
from eval_utils import get_ald
...
for step, data_batch in enumerate(pbar):
    ...
    pred_line = cv2.resize(preds_line, (config['img_width'],config['img_height']), interpolation=cv2.INTER_NEAREST)
    
    cv2.imwrite(f'{config["eval_gt"]}/{step}.png', (line_batch[0]*255).astype(np.uint8))
    cv2.imwrite(f'{config["eval_pred"]}/{step}.png', (pred_line*255).astype(np.uint8))
    

# %%
mald, mlld, mrld, mcont = get_ald(config['eval_gt'], config['eval_pred'])
print("\n")
print(f'Mean Average Lateral Distance: {mald}')
print(f'Mean Left Lateral Distance: {mlld}')
print(f'Mean Right Lateral Distance: {mrld}')
print(f'Mean Continuity: {mcont}')
# %%
