...
Geometric = iaa.Sequential(
    ...
    # iaa.OneOf([
        ...
        sometimes(iaa.Affine(rotate=(-15, 15), order=0, backend="cv2")),
...
def geomatric_augs(img_patch, depth, f_line, f_seg):
    #print('Geomatric AUG')
    ...
    depth = geom_aug.augment_image(depth)
    f_line = geom_aug.augment_image(f_line)
    f_seg = geom_aug.augment_image(f_seg)

    return img_patch, depth, f_line, f_seg

def noise_augs(img_patch, depth, f_line, f_seg):
    #print('Noise AUG')
...
def IDENTITY(img, depth, f_line, f_seg):
    pass
    return img, depth, f_line, f_seg
...
def data_augmenter(img_patch, depth, f_line, f_seg, epoch):
    
    func_args = [
                (geomatric_augs, (img_patch, depth, f_line, f_seg)),
                (noise_augs,     (img_patch, depth, f_line, f_seg)),
                (IDENTITY,       (img_patch, depth, f_line, f_seg))
                ]
    #print('one iter done')
    a, b, c = get_prob(epoch, E, p1, p2, p3)
    (func, args), = random.choices(func_args, weights=[a,b,c])
    img_patch, depth, f_line, f_seg = func(*args)                

    return img_patch, depth, f_line, f_seg, a, b
